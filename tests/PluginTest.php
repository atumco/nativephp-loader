...
/**
 * Plugin validation tests for NativephpLoader.
 *
...
describe('Plugin Manifest', function () {
    ...
    it('has required fields', function () {
        ...
        expect($manifest['name'])->toBe('atum/nativephp-loader');
        expect($manifest['namespace'])->toBe('NativephpLoader');
    });
...
describe('Native Code', function () {
    it('has Android Kotlin file', function () {
        $kotlinFile = $this->pluginPath . '/resources/android/NativephpLoaderFunctions.kt';

        expect(file_exists($kotlinFile))->toBeTrue();

        $content = file_get_contents($kotlinFile);
        expect($content)->toContain('package com.atum.plugins.nativephp_loader');
        expect($content)->toContain('object NativephpLoaderFunctions');
        expect($content)->toContain('BridgeFunction');
    });

    it('has iOS Swift file', function () {
        $swiftFile = $this->pluginPath . '/resources/ios/NativephpLoaderFunctions.swift';

        expect(file_exists($swiftFile))->toBeTrue();

        $content = file_get_contents($swiftFile);
        expect($content)->toContain('enum NativephpLoaderFunctions');
        expect($content)->toContain('BridgeFunction');
    });
...
describe('PHP Classes', function () {
    it('has service provider', function () {
        $file = $this->pluginPath . '/src/NativephpLoaderServiceProvider.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Atum\NativephpLoader');
        expect($content)->toContain('class NativephpLoaderServiceProvider');
    });

    it('has facade', function () {
        $file = $this->pluginPath . '/src/Facades/NativephpLoader.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Atum\NativephpLoader\Facades');
        expect($content)->toContain('class NativephpLoader extends Facade');
    });

    it('has main implementation class', function () {
        $file = $this->pluginPath . '/src/NativephpLoader.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Atum\NativephpLoader');
        expect($content)->toContain('class NativephpLoader');
    });
});
